...
char	*change_cmd_0(char **cmd, char *path1) //modif la cmd[0] pour ne pas avoir le path pour deuxieme argument de execve.
{
	int	i;
...
void	path_user(char *input, char **env)
{
	...
	cmd[0] = change_cmd_0(cmd, path1);
	if (execve(path1, cmd, env) == -1)
	{
		free_tab(cmd);
		free(path1);
		error_msg("error execve\n");
